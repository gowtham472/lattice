...
lattice verify lattice-1.0.1-x86_64-unknown-linux-musl.sbom.cdx.json --public-key release.pub
```
...
```bash
tar -xzf lattice-1.0.1-x86_64-unknown-linux-musl.tar.gz
sudo cp -r lattice-1.0.1-x86_64-unknown-linux-musl/{bin,share,lib} /usr/
lattice --version
...
```bash
docker build -f packaging/Dockerfile -t lattice:1.0.1 .
docker run --rm -v "$PWD/estate:/estate:ro" -v "$PWD/out:/out" lattice:1.0.1 \
